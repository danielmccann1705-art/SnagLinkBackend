...
`npm ci` and `npm run check` validate the adapter. Use permitted temporary npm/buildx caches on this workstation. The locked package versions, Dockerfile and `.dockerignore` keep dependency and secret handling reviewable. Request headers, cookies, binary bodies and redirects are preserved; proxy identity is replaced with Cloudflare's client address. Responses use no-store/no-referrer/noindex headers, except successful portal HTML uses `strict-origin` so Google Identity Services can recognise its registered origin. This policy sends no path, query or token even to same-origin resources; API, Contractor, errors and non-HTML assets remain `no-referrer`. See Google's [identity setup guidance](https://developers.google.com/identity/gsi/web/guides/get-google-api-clientid#content_security_policy). Persistent request/container logging is **off by default** on the staging candidate, because Cloudflare's invocation logs record the request URL and Snaglist's capability tokens live in URL paths. It is a switch: `SNAGLIST_CANDIDATE_LOGGING=on` at generation time turns the Worker's `observability` and the container's `logs` on together, which is what the B5 gate needs to read B2.1's per-write `kind:` lines — those lines are the container's stdout, and they reach the store only when the Worker's observability is on as well. `on` is narrowed to exactly that: it generates `observability.logs.invocation_logs = false`, so the container's `kind:` lines flow and request URLs — including the Contractor link path the gate itself drives — are deliberately not recorded. The decision, the reason and what it costs are recorded in one place — the note on `loggingVariable` in `scripts/candidate-config.mjs` — and this line does not restate them. Production and the recovery Worker keep logging off unconditionally in their own configurations.
