...
Dan selected **usesnaglist.com** on 11 September. Registration is authorised and prepared in Cloudflare at US$10.46 for one year (current renewal US$10.46/year); it is **not purchased yet**, pending his registrant telephone number and confirmation of the legal owner because the saved optional organisation is Reeve. Google Workspace Business Starter remains the email recommendation, with no paid subscription activated. Keep `snaglist.dev` and existing API/Contractor-link routing compatible. After verifying domain ownership, configure marketing, manager origin, email authentication and production provider registrations consistently; do not break already-issued links. See `DOMAIN-AND-EMAIL-RECOMMENDATION.md` for the current registration checkpoint.
