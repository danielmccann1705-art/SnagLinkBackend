...
(() => {
  ...
  function submission(item,draft) {
    ...
    return `<form class="submission" data-submit="${item.id}">
      ...
      <button type="button" class="secondary" data-choose="${item.id}" ${locked?'disabled':''}>Add after photos</button>
      ...
      <div class="actions"><button type="submit" class="primary" ${busy||stale?'disabled':''}>${busy?'Sending your fix…':draft.request?'Retry submission':'Submit for review'}</button><button type="button" class="secondary" data-cancel="${item.id}" ${busy?'disabled':''}>Back to snag</button></div>
