...
/// Stable register download, explicitly limited to project/snags. It does not
/// masquerade as the complete native graph bootstrap (media/directories/history).
struct RegisterSyncService {
    ...
    static func create(projectID: UUID, actorID: UUID, on db: Database) async throws -> RegisterSnapshotPage {
        ...
        guard count <= 10000 else { throw Abort(.payloadTooLarge, reason: "This project needs a background snapshot. No partial download was created", identifier: "snapshot_job_required") }
    ...
    static func page(token: String, offset: Int, projectID: UUID, actorID: UUID, on db: Database) async throws -> RegisterSnapshotPage {
        ...
        return .init(snapshotToken: token, coverage: ["project", "snags"], items: items, total: total, nextOffset: next, changesCursor: cursor, expiresAt: expiry)
    }
    static func changes(cursor: String, projectID: UUID, actorID: UUID, on db: Database) async throws -> ProjectChangePage {
        ...
        let rows = try await sql.raw("SELECT * FROM platform_changes WHERE workspace_id = \(bind: workspaceID) AND project_id = \(bind: projectID) AND sequence > \(bind: sequence) ORDER BY sequence LIMIT 101").all()
